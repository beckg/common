...
static TCP_SOCKET hCurrentTCP;     // Current TCP socket
...
{
    return data + TCPPutROMArray(hTCP, data, strlenpgm((ROM far char*)data));
}
...
static void TCPRAMCopyROM(PTR_BASE wDest, BYTE wDestType, ROM BYTE* wSource, WORD wLength)
{
    ...
    {
        case TCP_PIC_RAM:
            memcpypgm2ram((void*)(BYTE*)wDest, (ROM far void*)wSource, wLength);
            break;
        ...
        case TCP_ETH_RAM:
            ...
            while(wLength)
            {
                ...
                // Read and write a chunk   
                memcpypgm2ram(vBuffer, (ROM far void*)wSource, w);
                MACPutArray(vBuffer, w);
                wSource += w;
                wLength -= w;
            }
